    ...
    >
      <Container>
        ...
        <div className="mt-12 grid grid-cols-2 gap-4 max-bp-md:grid-cols-1">
          {items.map((it, i) => {
            const cardClass = "flex h-full items-start gap-5 rounded-card border border-grey2 bg-white p-7 shadow-soft transition-all duration-200 hover:-translate-y-0.5 hover:border-brand1 hover:shadow-md max-bp-sm:gap-4 max-bp-sm:p-5";
            const inner = (
              <>
                ...
                <div className="min-w-0 flex-1">
                  ...
                  {it.href && (
                    <span className="mt-2 inline-flex items-center gap-1 text-[12px] font-semibold text-brand1">
                      Открыть документ →
                    </span>
                  )}
                </div>
            ...
            return (
              <Reveal key={it.title} delay={((i % 3) + 1) as 1 | 2 | 3}>
                {it.href ? (
                  <a href={it.href} target="_blank" rel="noopener noreferrer" className={cardClass}>
                    {inner}
                  </a>
                ) : (
